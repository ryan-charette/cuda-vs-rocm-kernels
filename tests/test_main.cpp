...
#ifdef PGKL_HAS_HIP
auto hip_available() -> bool {
    int count = 0;
    return hipGetDeviceCount(&count) == hipSuccess && count > 0;
}

auto test_reduction_hip() -> bool {
    if (!hip_available()) {
        std::cout << "SKIP: reduction_hip (no HIP device)\n";
        return true;
    }

    const auto input = pgkl::make_patterned_vector(4096);
    const auto expected = pgkl::reduction_cpu(input);
    const auto got = pgkl::reduction_hip(input);
    if (!pgkl::nearly_equal(got, expected, 1.0e-4F, 1.0e-4F)) {
        std::cerr << "[reduction_hip] expected " << expected << ", got " << got << '\n';
        return false;
    }
    return true;
}

auto test_stencil_hip() -> bool {
    if (!hip_available()) {
        std::cout << "SKIP: stencil_hip (no HIP device)\n";
        return true;
    }
    ...
    pgkl::stencil2d_cpu(input, expected, 32, 32);
    pgkl::stencil2d_hip(input, got, 32, 32);

    std::size_t bad_index = 0;
    if (!pgkl::vectors_nearly_equal(got, expected, 1.0e-5F, 1.0e-5F, &bad_index)) {
        std::cerr << "[stencil_hip] mismatch at index " << bad_index << ": expected " << expected[bad_index]
                  << ", got " << got[bad_index] << '\n';
        return false;
    }
    return true;
...
auto test_matmul_hip() -> bool {
    if (!hip_available()) {
        std::cout << "SKIP: matmul_hip (no HIP device)\n";
        return true;
    }
    ...
    pgkl::matmul_tiled_cpu(a, b, expected, 32, 32, 32, 16);
    pgkl::matmul_tiled_hip(a, b, got, 32, 32, 32, 16);

    std::size_t bad_index = 0;
    if (!pgkl::vectors_nearly_equal(got, expected, 1.0e-4F, 1.0e-4F, &bad_index)) {
        std::cerr << "[matmul_hip] mismatch at index " << bad_index << ": expected " << expected[bad_index]
                  << ", got " << got[bad_index] << '\n';
        return false;
    }
    return true;
